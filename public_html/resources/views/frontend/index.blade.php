@extends('layouts.app') @section('title', 'Anasayfa') @section('slider')
<div id="start-screen" class="start-screen start-screen--style-1" data-scroll-discover="true">
                                    ...
                                    this.settings.firstTransition && this.first && (y = this.settings.firstTransition || y),
                                        ...
                                        h.css("transition", "all 0ms").each(function () {
                                            (this.className = "vegas-slide"),
                                                ...
                                                y && ((this.className += " vegas-transition-" + y), (this.className += " vegas-transition-" + y + "-in"));
                                        }),
                                        f._timer(!1),
                            ...
                            var slider = $("#vegas-slider"),
                                slides = [
                                    @foreach($sliders as $slider)
                                        {
                                            name: "img {{ $slider->id }}",
                                            src: "{{ asset(Storage::URL(str_replace('\\', '/', $slider->image))) }}",
                                        },
                                    @endforeach
    ...
    <div id="start-screen__content-container" class="start-screen__content-container text-white">
        @foreach ($sliders as $slider)
        <div class="start-screen__content__item start-screen__content__item--1 align-items-center" style="background: rgba(0, 0, 0, 0.5);">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-12 col-md-10 col-xl-8">
                        <div class="__name">{{ $slider->description }}</div>
                        <h2 class="__title text-white">{{ $slider->name }}</h2>
                        {{-- <p class="text-center">{{ $slider->description }}</p> --}}
                        <p class="text-center mt-5 mt-md-10">
                            @if($slider->link)
                            <span class="d-none d-sm-block"><a class="custom-btn custom-btn--big custom-btn--style-3" href="{{ $slider->link }}">Keşfet</a></span>
                            @endif
                        </p>
                    </div>
                </div>
    ...
    <section class="section">
        <div class="container">
            <div class="section-heading section-heading--center"">
                <h2 class="__title">Kategorilerimiz</h2>
            </div>
                                    ...
                                    <div class="col-6 col-md-4 col-lg-4">
                                        <a href="{{ route('product.index', ['category' => $category->slug]) }}">
                                            <div class="__item text-center">
                                                <i class="__ico">
                                                    <img class="img-fluid lazy" src="img/blank.gif" data-src="{{ asset(Storage::URL($category->thumbnail)) }}" alt="demo" />
                                                </i>
                                                <h5 class="__title">{{ $category->name }}</h5>
                                            </div>
                                        </a>
                                    </div>
                                @endforeach
    ...
    <section class="section section--no-pt section--no-pb">
        <div class="container">
            <div class="special-offer special-offer--style-1" data-aos="zoom-in" data-aos-duration="600" data-aos-offset="70">
                <h2 class="text text-center lazy" data-src="img/special_offer_text_bg.jpg">Maximum verim İÇİN SAĞLIKLI ÜRÜNLER</h2>
            </div>
        </div>
    </section>
</main>
